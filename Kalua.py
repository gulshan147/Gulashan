    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"Please specify a valid number and unit of time (hours/days) script by OWNER- @Gulshan_Alam..."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = f"ONLY OWNER CAN USE💀OWNER OWNER- @Gulshan_Alam..."
...
async def allusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        if users:
            ...
            response = f"No data found OWNER- @Gulshan_Alam ..."
    else:
        response = f"ONLY OWNER CAN USE.OWNER- @Gulshan_Alam..."
    await update.message.reply_text(response)
